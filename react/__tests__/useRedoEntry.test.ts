...
describe('useRedoEntry', () => {
  it('returns empty entry when handle is null', () => {
    const { result } = renderHook(() => useRedoEntry(null, 0));
    expect(result.current).toEqual({ index: -1, label: '' });
  });

  it('returns correct redo entry after undo', () => {
    const engine = createMockHistoryEngine();
    engine.push_command('{"label":"Action A"}');
    engine.push_command('{"label":"Action B"}');
    engine.undo();
    engine.undo();
    const handle = createHandle(engine);
    const { result } = renderHook(() => useRedoEntry(handle, 0));
    expect(result.current).toEqual({ index: 0, label: 'Action A' });
  });

  it('returns second redo entry', () => {
    const engine = createMockHistoryEngine();
    ...
    engine.undo();
    const handle = createHandle(engine);
    const { result } = renderHook(() => useRedoEntry(handle, 1));
    expect(result.current).toEqual({ index: 1, label: 'Second' });
  });
  ...
  it('updates on notify', () => {
    ...
    act(() => { handle.pushCommand('{"label":"X"}'); handle.undo(); });
    expect(result.current.label).toBe('X');
  });
});
