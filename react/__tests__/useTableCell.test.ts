...
describe('useTableCell', () => {
  it('returns empty CellState when handle is null', () => {
    const { result } = renderHook(() => useTableCell(null, 0, 'name'));
  ...
  it('returns correct cell state (value, error, dirty)', () => {
    ...
    const { result } = renderHook(() => useTableCell(handle, 2, 'price'));
  ...
  it('reflects edit value changes', () => {
    const engine = createMockTableEngine();
    const handle = createTableHandle(engine);

    const { result } = renderHook(() => useTableCell(handle, 0, 'name'));
  ...
  it('updates on notify', () => {
    ...
    const { result } = renderHook(() => useTableCell(handle, 1, 'status'));
    expect(result.current.value).toBe('');

    act(() => {
      engine._edits.set('1:status', 'active');
      engine._editErrors.set('1:status', 'Invalid status');
